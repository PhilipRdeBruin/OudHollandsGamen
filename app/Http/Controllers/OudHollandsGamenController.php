...
/*
    ...
    public function __construct(\App\Helpers\GeneralHelper $helper) {
        $this->helper = $helper;
        $this->helper->add(1,2);
    }
*/
    public function index() {
        ...
        $filterkey = (isset($_GET['filter'])) ? $_GET['filter'] : "filter";
    ...
    private function filteraan($filterkey) {
        // filtert op categorie, prijs of afstand

        echo '<script type="text/javascript">alert("' . $filterkey . '")</script>';

        return \Redirect::to('index');
    }

    public function filteren() {
        // filtert op categorie, prijs of afstand
        $filterkey = Input::get('filterkey');

        $advertentie = \App\Advertentie::where;
